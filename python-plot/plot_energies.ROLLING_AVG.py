...
def main():
  ...
  plt.plot(   inds, get_rolling_avgs(Ekins), color='g', label='E_kinetic (Ry), joined')
  plt.scatter(inds, get_rolling_avgs(Ekins), color='k', marker='.',  label='E_kinetic (Ry), data')
  ...
  plt.plot(   inds, get_rolling_avgs(diff_Etots_scaled), color='m', label='E_total[t] - E_total[0] = E_total[t] + 548.00599230 Ry', linewidth=1)
#  plt.scatter(times, diff_Etots_scaled, color='k', marker='.')
  ...
  #plt.ylim(ymin=-547.5)
  plt.savefig("Ekin_and_Etot_rolling-dt{}-nstep{}-nefgstep{}-nosym-ecut100.pdf".format(dt, nstep, nefgstep))
  plt.show()  
