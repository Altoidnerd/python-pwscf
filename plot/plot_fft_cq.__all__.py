...
# fourier
fourier24 = np.absolute(fft.rfft(signal24))
...
def main():
  # colors=bgrcmykw
  plt.plot(wavenumbers[:1000],     1/fourier1.size*fourier1[:1000],	label="Cq Cl1" , color="b",  linewidth=4   )
  plt.plot(wavenumbers[:1000],   1/fourier12.size*fourier12[:1000],	label="Cq Cl12", color="g",  linewidth=2   )
  plt.plot(wavenumbers[:1000],   1/fourier18.size*fourier18[:1000],	label="Cq Cl18", color="r",  linewidth=1   )
  plt.plot(wavenumbers[:1000],   1/fourier24.size*fourier24[:1000],	label="Cq Cl24", color="k",  linewidth=1   )
...
#  plt.xlabel("wavenumber (1/cm)")
  ...
  plt.legend(loc=1)
  plt.title('Spectrum of each Cq time series of Cl sites')  
  width_inches=7
  height_inches=3.5
  ...
  fig.savefig("fft_cq_eta.{:.3f}._each_cq_only_individually__.pdf".format(aspect))
