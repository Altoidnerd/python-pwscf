...
#  plt.plot(wavenumbers[:1000],   1/fourier24.size*fourier24[:1000],	label="Cq Cl24", color="k")
  plt.plot(wavenumbers[:1000],   1/efourier1.size*efourier1[:1000],	label="eta Cl1", color="b",  linewidth=4)
  plt.plot(wavenumbers[:1000], 1/efourier12.size*efourier12[:1000], 	label="eta Cl12",color="g",  linewidth=2)
  plt.plot(wavenumbers[:1000], 1/efourier18.size*efourier18[:1000], 	label="eta Cl1", color="r",  linewidth=1)
  plt.plot(wavenumbers[:1000], 1/efourier24.size*efourier24[:1000], 	label="eta Cl24",color="k",  linewidth=1)
  ...
  fig.savefig("fft_cq_eta.{:.3f}.__all_Clsites_individually__4321.pdf".format(aspect))
