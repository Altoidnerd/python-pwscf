...
def main():
  plt.plot(wavenumbers[:500], 1/fourier1.size*fourier1[:500], label="coupling constants", linewidth=2)
  plt.plot(wavenumbers[:500], 1/fourier2.size*fourier2[:500], label="asymmetry params", linewidth=2)
  plt.xlabel("wavenumber (1/cm)")
  plt.ylabel("Intensity (arb. units)")
  plt.legend(loc=1)
  plt.title('{}'.format(titlestring))  
  width_inches=7
  height_inches=3.5
  aspect=width_inches/height_inches

  titlestring = "Spectra of Cq, eta time series crystal (all Cl sites together)"
 
  fig = plt.gcf()
  fig.set_size_inches(20,8, forward=True)
  fig.savefig("fft_cq_eta.{:.3f}.__all_Cl_sites_together__.pdf".format(aspect))
