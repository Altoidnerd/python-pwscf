...
aCl1 = np.array( fix_broken_vector( [ np.abs(cq) for cq in Cl1 ],4 ))
aCl12= np.array( fix_broken_vector( [ np.abs(cq) for cq in Cl12],4 ))
aCl18= np.array( fix_broken_vector( [ np.abs(cq) for cq in Cl18],4 ))
aCl24= np.array( fix_broken_vector( [ np.abs(cq) for cq in Cl24],4 ))

Cl1 = np.array( fix_broken_vector( Cl1 ,4 ))
Cl12= np.array( fix_broken_vector( Cl12,4 ))
Cl18= np.array( fix_broken_vector( Cl18,4 ))
Cl24= np.array( fix_broken_vector( Cl24,4 ))

aeta1 = np.array( fix_broken_vector( [ np.abs(cq) for cq in eta1 ],4 ))
aeta12= np.array( fix_broken_vector( [ np.abs(cq) for cq in eta12],4 ))
aeta18= np.array( fix_broken_vector( [ np.abs(cq) for cq in eta18],4 ))
aeta24= np.array( fix_broken_vector( [ np.abs(cq) for cq in eta24],4 ))
...
def main():
  ...
  #plt.plot(   efg_inds, Cl1 , color='r', label='Cl1  (MHz)', linewidth=1)
  plt.scatter(efg_inds, Cl1 , color='r', label='Cl1  (MHz)', marker='.' )
  #plt.plot(   efg_inds, Cl12, color='c', label='Cl12 (MHz)', linewidth=1)
  plt.scatter(efg_inds, Cl12, color='c', label='Cl12 (MHz)', marker='.' ) 
  #plt.plot(   efg_inds, Cl18, color='y', label='Cl18 (MHz)', linewidth=1)
  plt.scatter(efg_inds, Cl18, color='y', label='Cl18 (MHz)', marker=',' )
  #plt.plot(   efg_inds, Cl24, color='g', label='Cl24 (MHz)', linewidth=1)
  plt.scatter(efg_inds, Cl24, color='g', label='Cl24 (MHz)', marker=',' )
...
##  plt.scatter(efg_inds, eta24, color='m', label='Cl24 (MHz)', marker=',' )
  ...
  plt.title("p-Cl_2-benzene MD, tempw={}K, dt={} a.u., nstep={}, T_avg={}K".format(tempw, dt, nefgstep,  get_mean(temperatures)))
  ...
  print(average_temp)
  plt.legend(loc=3)
  plt.savefig("cqs_no-abs-val-dt{}-nstep{}-nefgstep{}-nosym-ecut100.pdf".format(dt, nstep, nefgstep))
  #plt.ylim(ymin=-547.5)
